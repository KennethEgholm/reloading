...
- **Recipes**
  ...
  - **Load-Development Ladders** (`/recipes/ladders/new`): create a traditional ladder test in one step — pick the shared components once (caliber, projectile, propellant, primer, cartridge, rifle, COAL, notes) plus start charge / step / count (2–20 steps), with a live preview of every recipe that will be created. Saving runs one transaction that creates the ladder and N ordinary recipes (`{prefix} — {charge}gr`), each with its own charge.
    - Members are normal recipes: range sessions, chrono imports, MOA groups, load logs, and the AI check all work on them unchanged. In the recipes table they collapse under one ladder row (charge range + step count; click to expand). The recipe detail still shows a **Ladder** badge linking to the ladder page.
    - **Start from a recipe** (including one just imported from a QuickLOAD screenshot or `.dat`): the recipe detail has a **Start ladder** link that opens `/recipes/ladders/new?recipeId=…` with components, caliber, COAL, and start charge prefilled. Hidden once the recipe is already a ladder member.
    - The ladder page (`/recipes/ladders/[id]`) compares all members side by side: Charge | Avg MOA | Best MOA | Avg velocity | ES | SD | Sessions, aggregated from their range sessions and accuracy groups (`lib/ladder.ts`, pure + unit-tested). The best-MOA row is highlighted and one member can be marked **★ Winner**.
    - **Print**: recipe detail and the ladder page have a **Print** link (`/recipes/[id]/print`, `/recipes/ladders/[id]/print`) — a print-styled A4 sheet (browser print dialog / Save as PDF). Recipe card: large charge + COAL, components, and a blank **Measured V0** write-in. Ladder sheet: shared components plus a worksheet table (charge, blank V0, group mm, notes). Nav is hidden when printing.
    - **Delete ladder** removes the ladder and every step recipe. Load logs and range sessions keep their snapshots (the recipe link is cleared). To keep a step as a normal recipe, promote the winner first. Recipe export/import carries `ladderName` + `ladderChargeIndex` and re-links by ladder name (stub ladders are created if missing).
...
- **Rifles** (`/rifles`)
  ...
  - Recipe export/import includes the rifle as `{ name, caliber }` and re-links if a matching rifle exists (does not create stub rifles). Rifles themselves export/import from Settings → Data (match by name+caliber; specs update on match, create on miss). Everything import runs rifles before recipes so the re-link works on a empty DB.
