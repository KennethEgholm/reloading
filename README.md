...
- **Recipes**
  ...
  - **Load-Development Ladders** (`/recipes/ladders/new`): create a traditional ladder test in one step — pick the shared components once (caliber, projectile, propellant, primer, cartridge, rifle, COAL, notes) plus start charge / step / count (2–20 steps), with a live preview of every recipe that will be created. Saving runs one transaction that creates the ladder and N ordinary recipes (`{prefix} — {charge}gr`), each with its own charge.
    ...
    - **Promote winner**: after marking a winner, a button unlinks that step from the ladder, lets you rename it, and opens it as a normal recipe (load/range logs stay on it). Optionally delete the other steps at the same time (their logs keep their snapshots; the empty ladder is removed).
    - Deleting a ladder keeps every member recipe (only the grouping is removed). Recipe export/import carries `ladderName` + `ladderChargeIndex` and re-links by ladder name (stub ladders are created if missing).
