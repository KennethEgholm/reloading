...
import { extractQuickLoadFromImage, importRecipeFromQuickLoad, type QuickLoadImportData } from './actions'
...
interface QuickLoadImageImportProps {
  projectiles: ProjectileOption[]
  propellants: PropellantOption[]
  calibers: CaliberOption[]
}

const MAX_IMAGE_SIZE = 10 * 1024 * 1024 // 10MB, matches the server action guard

export function QuickLoadImageImport({ projectiles, propellants, calibers }: QuickLoadImageImportProps) {
  ...
  const handleFile = async (file: File | null) => {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      setError(t('qlImageImport.errors.tooLarge'))
      toast.error(t('qlImageImport.errors.tooLarge'))
      return
    }
    ...
    try {
      ...
      fd.set('image', file)
      const result = await extractQuickLoadFromImage(fd)
      setParsed(result)
      setEditable({
        name: result.name,
      ...
      const message = e instanceof Error ? e.message : t('qlImageImport.failed')
      setError(message)
      toast.error(message)
    } finally {
      setIsExtracting(false)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t('qlImageImport.success'))
      closeModal()
    } catch {
      toast.error(t('qlImageImport.failed'))
    } finally {
      setIsSaving(false)
    }
  }
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={closeModal}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="ql-image-import-title"
          ...
          >
            <h2 id="ql-image-import-title" className="text-xl font-semibold mb-4">{t('qlImageImport.title')}</h2>
            <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">{t('qlImageImport.hint')}</p>
            ...
            <input
              ...
              className="text-sm file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-medium file:bg-zinc-100 dark:file:bg-zinc-800 mb-4 disabled:opacity-50"
            />

            {isExtracting && (
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4" aria-live="polite">{t('qlImageImport.extracting')}</p>
            )}
            ...
            {parsed && (
              <div className="space-y-4" aria-live="polite">
                <p className="text-sm font-medium">{t('qlImageImport.preview')}</p>
