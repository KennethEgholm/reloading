...
- Load logs: full denormalized snapshots on the log row — name/caliber/charge/**COAL**/projectile/propellant/primer/**cartridge** (brand/caliber/water capacity)/V0/fillRate — plus the projectile/propellant/primer IDs for restore-on-delete. Delete uses a transaction that restores using the snapshot values (cartridge is snapshotted but not consumed, so it isn't restored). Range logs snapshot the same set **minus cartridge** (deliberate scope choice — add it to RangeLog for full parity if desired).
